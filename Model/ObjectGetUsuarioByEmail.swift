...
struct DatosRepuestaGetUsuario : Decodable{
    ...
    init(from decoder: Decoder) throws {
        ...
        aplicaciones = try container.decodeIfPresent([FailableDecodable<AplicacionesGetUsuario>].self, forKey: .aplicaciones)?
        .compactMap { $0.base } // .flatMap in Swift 4.0
        countrySourceId = try container.decodeIfPresent(String.self, forKey: .countrySourceId)
        direccion = try container.decodeIfPresent(DireccionUsuario.self, forKey: .direccion)//try DireccionLogin(from: DireccionLogin.self as! Decoder)
        estado = try container.decodeIfPresent(EstadoUsuario.self, forKey: .estado)//try EstadoLogin(from: EstadoLogin.self as! Decoder)
        imagen = try container.decodeIfPresent(ImagenUsuario.self, forKey: .imagen)//try ImagenLogin(from: ImagenLogin.self as! Decoder)
        cuenta = try container.decodeIfPresent(CuentaUsuario.self, forKey: .cuenta)//try CuentaLogin(from: CuentaLogin.self as! Decoder )
        ...
        ocupacion = try container.decodeIfPresent(OcupacionUsuario.self, forKey: .ocupacion)//try OcupacionLogin(from: OcupacionLogin.self as! Decoder )
        tipoDocumento = try container.decodeIfPresent(TipoDocumentoUsuario.self, forKey: .tipoDocumento) //try TipoDocumentoLogin(from: TipoDocumentoLogin.self as! Decoder)
        numeroDocumento = try container.decodeIfPresent(String.self, forKey: .numeroDocumento)
        perfilAloRrp = try container.decodeIfPresent(PerfilAloRrpUsuario.self, forKey: .perfilAloRrp)//try PerfilAloRrpLogin(from: PerfilAloRrpLogin.self as! Decoder)
